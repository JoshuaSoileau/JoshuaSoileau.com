...
Sometimes I get so angry 💢 that an emoji doesn't do it justice.
...
## What really is ASCII, and how can we animate it? 🤔
...
{`<span className="body">
  ( // (
  <span className="arm">╯</span> // (╯ <span className="eye">°</span> // (╯°
  <span className="mouth">□</span> // (╯°□ <span className="eye">°</span> //
...
## 🚫 Sidenote: Animations can be scary.
...
## 1️⃣ 2️⃣ 3️⃣ <br/><br/>The steps for animating our ASCII art

We can break down the process of animating <small className="mx-2 inline-block">(╯°□°)╯︵┻━┻</small> into the following:
...
Notice how the face literally changes it's characters. The contents in those spans changes from `°□°` to `—_—` and `>.<`. This is different then what I laid out at the beginning of this post, where I said we would lay each character out inside of it's own `<span>` tag.
